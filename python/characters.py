...
def create_first_task(form, images):
    ...
    TRAINING_SAMPLES_PER_TASK = int(form.getfirst('trainingSamplesPerTask', 1))
    TESTING_IMAGES_PER_TASK = int(form.getfirst('testingImagesPerTask', 10))
    ...
    TRUE_TESTING_IMAGES_PER_TASK = TESTING_IMAGES_PER_TASK - TOTAL_DISTRACTORS_PER_TASK
    
    alphabets = get_accepted_image_list(from_path=FROM_PATH)

    all_characters = [(alphabet, image_num) for image_num in range(len(alphabets[alphabet].values()[0])) for alphabet in alphabets]
    ...
    for i, (alphabet, character_num) in enumerate(training_characters):
        ...
        same_alphabet_other_characters = [alphabets[alphabet][uid][character_num]
        ...
        other_alphabet_other_characters = [alphabets[alphabet][uid][character_num]
                                           for num in range(len(alphabets[alphabet][uid]))
                                           for uid in sorted(alphabets[alphabet].keys())
        ...
        tasks.append({'training alphabet':alphabet,
                      ...
                      'training characters':list(map(anonymize_image, same_alphabet_same_characters[:TRAINING_SAMPLES_PER_TASK])),
                      'test characters': list(map(anonymize_image,
                                                  random.sample((same_alphabet_same_characters[TRAINING_SAMPLES_PER_TASK:TRAINING_SAMPLES_PER_TASK+TRUE_TESTING_IMAGES_PER_TASK] +
