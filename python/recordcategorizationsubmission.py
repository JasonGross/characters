...
def _make_summary(properties):
    ...
    for group_num, group in groups:
        ...
        for question_num, question in group:
            ...
            question['selected alphabet id'] = [choice_alphabets[int(value)] for value, key in properties if selection_regex.match(key)][0]
...
def _put_summary(folder, properties, file_name):
    push_dir(folder)
...
def record_submission(form_dict, many_dirs=False, path=UNREVIEWED_PATH,
    ...
    if not pseudo:
        ...
        _put_properties(path, form_dict, _make_file_name(uid, summary=True), quiet=quiet)
    if many_dirs:
        _log_success(folder)
