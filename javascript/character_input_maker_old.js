...
function makeImg(tag)
{
  ...
  if (getURLParameter('image'+tag) != '') {
    rtn = rtn + '<img src="'+getURLParameter('image'+tag)+'"';
...
function makeCanvasWidth(param, tag, canvasId)
{
  ...
  if (getURLParameter(param).toLowerCase() == 'fit' ||
    ...
    if (img.width != 0) rtn = img.width;
      ...
      if (img.attachEvent) {window.attachEvent('onload', function(){fixW();});
                            img.attachEvent('onload', function(){fixW();});}
      else if (img.addEventListener) {window.addEventListener('load', function(){fixW();}, false);
                                     img.addEventListener('load', function(){fixW();}, false);}
      else {img.addEventListener('load', function(){fixW();}, false);}
    }
...
function makeCanvasHeight(param, tag, canvasId)
{
  ...
  if (getURLParameter(param).toLowerCase() == 'fit' ||
    ...
    if (img.height != 0) rtn = img.height;
    else {
      rtn = backupCanvasHeight;
      var fixH = function() { document.getElementById(canvasId).height = img.height; };
      if (img.attachEvent) {window.attachEvent('onload', function(){fixH();});
                            img.attachEvent('onload', function(){fixH();});}
      else if (img.addEventListener) {window.addEventListener('load', function(){fixH();}, false);
                                      img.addEventListener('load', function(){fixH();}, false);}
      else {img.addEventListener('load', function(){fixH();}, false);}
    }
  } else if (getURLParameter(param) != '')
    rtn = getURLParameter(param);
  else
...
function makeCharacterInput(name, tag)
{
  ...
  rtn += '</canvas></div>';
  rtn += '<div id="canvascontrols"> <a id="clear" href="javascript:'+name+'.clear()" unselectable="on">clear</a>';
  rtn += '<a id="undo" name="'+name+'_undo" href="javascript:'+name+'.undo()" unselectable="on"></a>';
  rtn += '<a id="redo" name="'+name+'_redo" href="javascript:'+name+'.redo()" unselectable="on"></a>';
  rtn += '</div>';
