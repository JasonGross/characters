(function ($, jQuery, undefined) {
  ...
  var allUrlParameters = ['taskCount', 'imageHalf', 'characterSize', 'characterSet'];

  function retrieveData(loadData) {
    $.getJSON("../scripts/python/completion-characters.py",
...
//      $('.expected-duration').html((data['tasks'].length / 10) + '-' + (Math.floor(data['tasks'].length / 2.5))); // minutes
    ...
    tasks = new CompletionTasks(data, data['tasks'].length, dataLoader, onDoneTasks);
    doneLoading();
  };
  notYetLoaded();
