...
public class UpdateQualificationType extends AbstractCmd {
  ...
  protected void runCommand(CommandLine cmdLine) throws Exception {
      ...
      if (cmdLine.hasOption(ARG_QUALTYPE)) {
          ...
          try {
              ...
              log.error("error occured reading file: " + e.getLocalizedMessage());
              System.exit(-1);
          }
      ...
      try {
        ...
        log.error("Error updating qualification type: " + e.getLocalizedMessage());
        System.exit(-1);
      }
  }
      ...
      String propertiesFile ) throws Exception
  {
    ...
    try {
      ...
      log.error( "Couldn't find one of the specified files: " + e.getLocalizedMessage());
    } catch (IOException e ) {
      log.error( "Error reading one of the specified files: " + e.getLocalizedMessage());
    }
    ...
    try {
      ...
      log.error( "Failed to load properties file: " + e.getLocalizedMessage());
    }
