...
def _make_folder_for_submission(uid, path=CLASSIFICATION_UNREVIEWED_PATH, return_num=True):
...
def _make_summary(properties):
    ...
    task_start_time = 'task-%d-time-of-show-classes'
    task_end_time = 'task-%d-time-of-finish-task'
    task_desc = 'GIVEN %%(task-%(task)d-anchor-image-0-alphabet)s, %%(task-%(task)d-anchor-image-0-character-number)s, %%(task-%(task)d-anchor-image-0-id)s, ' + \
...
def record_submission(form_dict, many_dirs=True, path=CLASSIFICATION_UNREVIEWED_PATH,
