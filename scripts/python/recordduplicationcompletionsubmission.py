#!/usr/bin/python
# recordduplicationcompletionsubmission.py -- Stores the data from the duplication/completion submission
import os, tempfile
import cairo
...
def record_submission(form_dict, path=CLASSIFICATION_UNREVIEWED_PATH, **kwargs):
    desc =  'FLASHED %%(task-%(task)d-duplication-image-alphabet)s, %%(task-%(task)d-duplication-image-character-number)s, %%(task-%(task)d-duplication-image-id)s'
    desc +=  ' and GIVEN %%(task-%(task)d-completion-image-alphabet)s, %%(task-%(task)d-completion-image-character-number)s, %%(task-%(task)d-completion-image-id)s'
